    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--color-muted-foreground)]">
          Describe a task, choose the agents (or let an orchestrator plan it), and watch them collaborate live.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[380px_1fr]">
        ...
        <form onSubmit={go} className="h-fit space-y-4 rounded-[var(--radius)] border border-[var(--color-border)] bg-[var(--color-card)] p-4">
          <div>
            ...
            <textarea className="w-full rounded-md border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2 text-sm"
              rows={3} placeholder="e.g. Research the latest on AI agents and write a 3-bullet brief"
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setMode("pipeline")}
                className={`flex-1 rounded-md border px-3 py-2 text-sm ${mode === "pipeline" ? "border-[var(--color-primary)]" : "border-[var(--color-border)]"}`}>
                Pipeline <span className="block text-[10px] text-[var(--color-muted-foreground)]">selected agents, in order</span>
              </button>
              <button type="button" onClick={() => setMode("auto")}
                className={`flex-1 rounded-md border px-3 py-2 text-sm ${mode === "auto" ? "border-[var(--color-primary)]" : "border-[var(--color-border)]"}`}>
                Auto <span className="block text-[10px] text-[var(--color-muted-foreground)]">orchestrator delegates</span>
              </button>
            </div>
          </div>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-[var(--color-muted-foreground)]">
              Agents {mode === "auto" && "(the orchestrator may delegate to these)"}
            </label>
              ...
              {agents.map((a, i) => (
                <label key={a.id} className="flex items-center gap-2 rounded-md border border-[var(--color-border)] p-2 text-sm">
                  ...
                  <span className="flex-1">
                    ...
                    <span className="text-xs text-[var(--color-muted-foreground)]"> — {a.role}</span>
                  </span>
